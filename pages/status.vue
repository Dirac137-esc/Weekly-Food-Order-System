<template>
  <v-container class="status-bg py-10" fluid>
    <v-row justify="center">
      <v-col cols="12" md="10">
        <div class="mb-8">
          <div class="text-h5 font-weight-bold mb-1 mt-15 status-title">Захиалгын дэлгэрэнгүй</div>
          ...
          <v-row>
            ...
            <v-col cols="12" md="4">
              <v-card class="pa-4 mb-2 status-info-card" elevation="2">
                ...
                <div class="d-flex align-center">
                  ...
                  <span>**** 1234</span>
                  <span class="ml-2">{{ items.reduce((s, i) => s + i.qty * i.price, 0) }}₮</span>
                </div>
            ...
            <v-col cols="12" md="4">
              <v-card class="pa-4 mb-2 status-info-card" elevation="2">
                ...
                <div>Нийлбэр: {{ items.reduce((s, i) => s + i.qty * i.price, 0) }}₮</div>
                <div>Хүргэлт: Үнэгүй</div>
                <div class="font-weight-bold mt-1">Нийт: {{ items.reduce((s, i) => s + i.qty * i.price, 0) }}₮</div>
              </v-card>
            </v-col>
          </v-row>
        </div>
        ...
        <v-card class="pa-6 mb-6 status-main-card" elevation="8">
          ...
          <v-row>
            <v-col cols="12" md="8">
              <v-list dense>
                <v-list-item v-for="item in items" :key="item.id" class="status-food-item">
                  <v-list-item-avatar>
                    <v-img :src="item.img" />
                  </v-list-item-avatar>
                  <v-list-item-content>
