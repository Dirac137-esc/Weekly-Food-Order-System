<template>
  <v-container fluid class="auth-bg" style="min-height:100vh;padding-top:40px;position:relative">
    <div class="auth-overlay"></div>
    <v-row class="fill-height" no-gutters>
      <v-col cols="12" md="6" class="d-flex flex-column justify-center align-center text-center quote-col" style="z-index:2">
        <div class="left-side">
      ...
      <v-col cols="12" md="6" sm="12" class="d-flex justify-center align-center" style="z-index:2;min-height:100vh;margin-left:50vw">
        <v-card max-width="600" class="pa-6 mt-10 right-card auth-card mr-5" elevation="10">
          <transition name="slide-fade" mode="out-in">
            <v-row :key="isLogin" class="mb-6 justify-center">
              <v-btn-toggle v-model="isLogin" class="login-toggle" mandatory rounded-4xl group>
                <v-btn :value="true" class="toggle-btn" color="primary" variant="flat" size="large" elevation="4">
                  &nbsp;Нэвтрэх &nbsp;&nbsp;
                </v-btn>
                <v-btn :value="false" class="toggle-btn" color="success" variant="flat" size="large" elevation="4">
                  Бүртгүүлэх
                </v-btn>
              </v-btn-toggle>
            </v-row>
          ...
          <transition name="slide-fade" mode="out-in">
            <div :key="isLogin">
              <v-form v-if="isLogin" @submit.prevent="login">
                ...
                <v-text-field v-model="loginEmail" label="Хэрэглэгчийн Email" prepend-inner-icon="mdi-account" autocomplete="email" required />
                ...
                <div class="d-flex align-center justify-space-between mb-3">
                  <v-checkbox v-model="loginRememberMe" label="Намайг санах" class="ma-0 pa-0 remember-checkbox" hide-details density="compact"/>
                  <v-btn variant="text" color="grey" size="small" class="forgot-btn" @click="forgotPassword">Нууц үгээ мартсан</v-btn>
                </div>
                <div class="text-center">
                  <v-alert v-if="loginResult" :type="loginResult.includes('Амжилттай') ? 'success' : 'error'" class="mb-4">{{ loginResult }}</v-alert>
                </div>
                <v-btn color="primary" type="submit" block class="main-action-btn" elevation="6">Нэвтрэх</v-btn>
              </v-form>
              <v-form v-else @submit.prevent="register">
                ...
                <div class="d-flex align-center mb-3">
                  <v-checkbox v-model="registerRememberMe" label="Намайг санах" class="ma-0 pa-0 remember-checkbox" hide-details density="compact"/>
                </div>
                <div class="text-center">
                  <v-alert v-if="registerResult" :type="registerResult.includes('Амжилттай') ? 'success' : 'error'" class="mb-4">{{ registerResult }}</v-alert>
                </div>
                <v-btn color="#4BB543" type="submit" block class="main-action-btn" elevation="6">Бүртгүүлэх</v-btn>
              </v-form>
            </div>
          </transition>
        </v-card>
        <div class="action-section-outside mr-10 d-flex flex-column" style="height:30%;">
          <div class="text-center mb-4" style="color:#666;font-size:0.9rem;">Та мөн үүгээр нэвтрэх боломжтой</div>
          <v-row no-gutters>
            <v-col cols="12">
...
async function login() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (loginRememberMe.value) localStorage.setItem("rememberedEmail", loginEmail.value);
...
// -------------------
function forgotPassword() { alert("Нууц үгээ мартсан уу?"); }
function loginWithGoogle() { alert("Google-р нэвтрэх"); }
function loginWithFacebook() { alert("Facebook-р нэвтрэх"); }
function registerWithGoogle() { alert("Google-р бүртгүүлэх"); }
function registerWithFacebook() { alert("Facebook-р бүртгүүлэх"); }
</script>
...
@media (max-width:960px){
  ...
  .action-section-outside{margin-top:24px!important;padding:12px 4px 8px 4px!important;}
