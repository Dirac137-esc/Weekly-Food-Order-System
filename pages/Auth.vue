            ...
            <div :key="isLogin">
              ...
              <v-form v-if="isLogin" @submit.prevent="login">
                ...
                <div class="text-center">
                  <v-alert v-if="loginResult" :type="loginResult.includes('Амжилттай') ? 'success' : 'error'" class="mb-4">
                    {{ loginResult }}
                  </v-alert>
                </div>
              </v-form>
              ...
              <v-form v-else @submit.prevent="register">
                ...
                <div class="text-center">
                  <v-alert v-if="registerResult" :type="registerResult.includes('Амжилттай') ? 'success' : 'error'" class="mb-4">
                    {{ registerResult }}
                  </v-alert>
                </div>
              </v-form>
...
async function login() {
  loginResult.value = ''
  try {
    const response = await fetch('https://backend-production-88df.up.railway.app/auth/login', {
      method: 'POST',
...
async function register() {
  ...
  try {
    const response = await fetch('https://backend-production-88df.up.railway.app/auth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
