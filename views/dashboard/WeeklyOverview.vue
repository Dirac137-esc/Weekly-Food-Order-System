...
const fetchStats = async () => {
  ...
  try {
    ...
    changePct.value =
      sumPrev === 0 ? 100 : Math.round(((sumThis - sumPrev) / sumPrev) * 100);
...
const options = computed(() => {
  ...
  const disabledColor = `rgba(${hexToRgb(c["on-surface"])},${
    v["disabled-opacity"]
  })`;
  const borderColor = `rgba(${hexToRgb(String(v["border-color"]))},${
    v["border-opacity"]
  })`;
