        ...
        >
          ...
          <template #item.paymentStatus="{ item }">
            <v-select
              ...
              variant="underlined"
              @update:model-value="
                (val) => updateStatus(item, 'paymentStatus', val)
              "
            >
          ...
          <template #item.deliveryStatus="{ item }">
            <v-select
              ...
              variant="underlined"
              @update:model-value="
                (val) => updateStatus(item, 'deliveryStatus', val)
              "
            >
              <template #selection="{ item: sel }">
                <v-chip
...
watch(
  ...
  (val) => {
    if (val.length === 2) {
      const [start, end] = val;
      filters.value.dateRangeFormatted = `${new Date(start).toLocaleDateString(
        "mn-MN"
      )} – ${new Date(end).toLocaleDateString("mn-MN")}`;
    } else {
...
const filteredOrders = computed(() =>
  orders.value.filter((o) => {
    ...
    if (
      filters.value.weekday &&
      mongolianWeekdays[o.weekday] !== filters.value.weekday
    )
      return false;

    if (
      (filters.value.minTotal != null &&
        o.totalCost < filters.value.minTotal) ||
    ...
    if (
      ...
      o.paymentStatus !== filters.value.paymentStatus
    )
      return false;
    return !(
      filters.value.deliveryStatus &&
      o.deliveryStatus !== filters.value.deliveryStatus
    );
  })
);
