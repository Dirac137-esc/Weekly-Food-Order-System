<template>
  <v-container class="py-6">
    <v-card elevation="2">
      ...
      <v-card-text>
        <v-alert v-if="msg" :type="msgType" class="mb-4" border="start" variant="tonal">
          {{ msg }}
        </v-alert>
  ...
  ×{{ item.qty }}
            ...
            <td style="max-width: 140px">
              <v-select
                  ...
                  hide-details
                  variant="underlined"
                  @update:model-value="val => updateStatus(order, 'paymentStatus', val)"
              >
            ...
            <td style="max-width: 160px">
              <v-select
                  ...
                  hide-details
                  variant="underlined"
                  @update:model-value="val => updateStatus(order, 'deliveryStatus', val)"
              >
                <template #selection="{ item }">
                  <v-chip
                      :color="getStatusColor(item.raw)"
...
async function updateStatus(order: Order, field: 'paymentStatus' | 'deliveryStatus', value: string) {
  try {
